...
const MovieDetails = ({ data }) => {
  ...
  const showsByDate = data.shows?.reduce((res, show) => {
  ...
  const showsByLocation = showsByDate[selectedDate]?.reduce((res, show) => {
  ...
  return <>
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-4 md:gap-6 text-base md:text-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-500">{format(new Date(data.releaseDate), 'MMMM d, yyyy')}</span>
          </div>
          <div className="flex items-center gap-2">
    ...
    <div className="mb-8">
      ...
      <p className="pb-4">Select a show date and time to continue to ticket checkout.</p>
      <div className="mb-2">
          ...
          <>
            ...
            <div className="flex flex-wrap gap-3">
              {shows.map((show) => (
                <button
                  key={show.id}
                  className={`rounded-xl border-2 py-1.5 px-4 transition-colors cursor-pointer ${show.id == selectedShowtime ? 'bg-blue-500 border-blue-500 text-white' : 'border-gray-300 hover:bg-gray-100'}`}
                  onClick={() => {
                    if (selectedShowtime == show.id) setSelectedShowtime(null);
      ...
      <div className="flex justify-center">
        <Button
          ...
          href={selectedShowtime ? `/checkout?movieId=${data.id}&showId=${selectedShowtime}` : null}
        >
