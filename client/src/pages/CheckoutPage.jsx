...
const CheckoutPage = () => {
  ...
  const fetchShowData = async () => {
    try {
      const response = await fetch(`https://www.moviebookingsystem.xyz/api/shows/${showId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      if (!response.ok) throw new Error('Failed to fetch movie or show data' + response.status);
  ...
  if (isAuthenticated == null || !showData) return <div className="flex justify-center py-40"><LoadingCircle className="w-8 h-8"/></div>;
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto py-8 px-4 flex flex-col">
        ...
        <div className="grid lg:grid-cols-[70%_1fr] mt-4">
          ...
          {/* checkout form */}
          <div className="flex flex-col gap-4 py-4 w-100">
            <Button
              variant="primary"
